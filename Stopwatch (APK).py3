...
class StopWatch(Frame):
    ...
    def buatTeks(self):
        self.teks = Label(self, textvariable=self.waktuString,font="Verdana 19 bold", bg='grey', fg='black')
    ...
    def Start(self):
        if not self.sedangBerjalan and self.textStart.get() == 'Start' :
            self.textStart.set('Split')
            self._start = time.time() - self.waktuSekarang
            self.perbarui()
            self.sedangBerjalan = True
        elif self.sedangBerjalan and self.textStart.get() == 'Split' :
